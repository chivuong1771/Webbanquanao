        ...
        <c:forEach var="cat" items="${applicationScope.categories}">
            <a href="${pageContext.request.contextPath}/products?categoryId=${cat.id}" class="category-card">
                <div class="category-icon">
                        <%-- SỬA LỖI 1: Đổi cat.name -> cat.categoryName --%>
                    <c:choose>
                        <c:when test="${cat.categoryName.contains('Áo Nam') || cat.categoryName.contains('Áo Nữ')}">
                            <i class="fa-solid fa-shirt"></i>
                        </c:when>
                        <c:when test="${cat.categoryName.contains('Quần')}">
                            <i class="fa-solid fa-socks"></i>
                        </c:when>
                        <c:when test="${cat.categoryName.contains('Phụ Kiện')}">
            ...
            <c:forEach var="prod" items="${discountProducts}">
                <div class="product-card">
                    ...
                    <div class="product-image-wrapper">
                            <%-- SỬA LỖI 2: Đổi prod.primaryImage -> prod.thumbnail --%>
                        <c:choose>
                    ...
                    <div class="product-info">
                            <%-- SỬA LỖI 3 & 4: Đổi prod.brandName -> prod.brandID.brandName và prod.name -> prod.productName --%>
                        <div class="product-brand">${prod.brandID.brandName}</div>
                        <h4 class="product-name"><a href="${pageContext.request.contextPath}/product-detail?id=${prod.id}">${prod.productName}</a></h4>
